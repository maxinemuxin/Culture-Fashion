...
if __name__ == "__main__":
    ...
    with open(CSV_PATH, "r") as f:
        ...
        for row in reader:
            key = row[0]
            value = row[1]
            african_label_dict[key] = value
    for i, img_path in enumerate(img_paths_african):
        name = img_path.split("/")[-1]
        caption = african_label_dict[name]
        d[img_path] = caption


    # get indian data
    IMG_ROOT = os.path.join("archive", "images","test")
    CSV_PATH = "indo_image_label.csv"
    ...
    with open(CSV_PATH, "r") as f:
        ...
        for row in reader:
            key = row[0]
            value = row[1]
            indo_label_dict[key] = value

    img_paths_indo_trun = []

    for i, img_path in enumerate(img_paths_indo):
        name = img_path.split("/")[-1]
        if name in indo_label_dict:
            caption = indo_label_dict[name]
            d[img_path] = caption
            img_paths_indo_trun.append(img_path)


    # split validation      
    img_paths = img_paths_african+img_paths_indo_trun
...
def load_model(model_path):
    """
    Load the pretrained CLIP model with the saved weights.

    Parameters:
        model_path (str): Path to the saved model file.

    Returns:
        CLIP model loaded with the best weights.
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model, preprocess = clip.load("ViT-B/32", device=device, jit=False)
